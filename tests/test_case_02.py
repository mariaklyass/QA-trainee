...
class TestCase02:
    def test_case_02(self, page: Page) -> None:
        def create_route_function(path):
            return lambda route: route.fulfill(path=path)
        page.route(API.HTTP_REQUEST, create_route_function("../data/json/test_case_2.json"))
        ...
        for i, counter in enumerate(filtered_counters):
            counter.screenshot(path=f"../output/test_case_2_counter_{i + 1}.png")
